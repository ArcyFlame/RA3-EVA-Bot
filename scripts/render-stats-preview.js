...
const { generateBarChart } = require('../dist/utils/charts');
...
async function main() {
  ...
  const charts = [
    [
      'online_players_last_24_hours.png',
      stats.online_last_24h,
      'Online Players (Last 24 Hours)',
      'Reds_r',
    ],
    [
      'new_players_last_30_days.png',
      stats.new_players_last_30d,
      'New Players (Last 30 Days)',
      'Blues_r',
    ],
    [
      'online_players_last_30_days.png',
      stats.online_last_30d,
      'Online Players (Last 30 Days)',
      'YlOrBr_r',
    ],
  ];
  for (const [filename, values, title, palette] of charts) {
    const buffer = await generateBarChart(values, title, palette);
