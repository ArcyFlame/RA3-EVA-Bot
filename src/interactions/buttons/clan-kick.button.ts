...
export const customIdPrefix = 'clan_kick_';
...
export async function execute(_bot: RA3Bot, interaction: ButtonInteraction) {
  ...
  if (options.length === 0) {
    await interaction.editReply({ content: 'No kickable members found.' });
    return;
  }
  const select = new StringSelectMenuBuilder()
    .setCustomId(`clan_kick_select_${clanId}`)
    .setPlaceholder('Select member to kick')
    .addOptions(options);
  const row = new ActionRowBuilder<StringSelectMenuBuilder>().addComponents(select);
  await interaction.editReply({ content: 'Select a member to kick:', components: [row] });
}
